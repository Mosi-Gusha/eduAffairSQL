...
- `semesters`：学期名称、开始日期、结束日期、当前学期标记和最大学分。
...
- 维护学期：新增学期、编辑当前学期起止日期和最大学分、设置当前学期。
...
系统仍使用 `is_current` 标记当前学期，使用 `max_credit` 保存该学期的选课最大学分。选课除要求课程班状态为 `selecting` 外，还要求目标学期是当前学期且当前日期位于该学期起止日期内。
