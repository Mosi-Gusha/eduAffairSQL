...
- `course_offerings`、`course_offering_times`、`classrooms`：课程班、课程班上课时间段、教室和排课信息；已选人数由 `course_offering_stats` 视图动态统计。
- `enrollments`：学生选课记录。
...
- 维护课程班：为当前学期新增和调整课程班，课程班包含教师、教室、一个或多个上课时间段、容量和期末占比；课程班选课名单中可由管理员手动删除学生选课记录。
...
- 成绩登记面向可录入成绩的课程班，录入平时分和考试分后通过视图查看最终成绩与绩点。
...
- 课程班是具体开课安排，关联课程、学期、教师、教室、容量和成绩比例；上课时间段独立保存在 `course_offering_times`，选课容量由数据库触发器限制。
