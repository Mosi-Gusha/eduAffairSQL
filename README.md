...
这是一个面向学校日常教务场景的 B/S 管理系统，覆盖管理员、教师、学生三类角色，支持学期维护、课程基础信息维护、课程班安排、在线选课、成绩登记、成绩查询、考勤和通知公告等流程。
...
- `semesters`：只保存学期名称、开始日期、结束日期和当前学期标记，不单独保存学期状态。
- `courses`：课程基础信息，包括课程号、课程名、开课院系、学分和课程状态
- `course_offerings`、`classrooms`：课程班、教室和排课信息。
- `enrollments`：学生选课记录。
- `grades`：平时分、考试分、最终成绩和绩点。最终成绩为整数。
- `attendance`：考勤记录。
- `notices`、`system_settings`：通知和系统配置。
...
- 维护学期：新增学期、编辑当前学期起止日期、设置当前学期。
- 维护课程：查看课程号、课程名、开课院系、学分、课程状态；可新增课程、启用课程、弃用课程，不提供编辑或删除。
- 维护课程班：为当前学期新增和调整课程班，课程班包含教师、教室、星期、节次、周次、容量和平时/考试比例。
- 弃用课程不能再新建课程班；已创建的历史课程班不受影响。
- 维护学生和教师。学生管理展示学院、专业、学生邮箱、入学年份和账号状态。
- 发布、编辑和删除通知。
...
系统仍使用 `is_current` 标记当前学期。选课除要求课程班状态为 `selecting` 外，还要求目标学期是当前学期且当前日期位于该学期起止日期内。

## 课程与课程班规则

- 课程是基础数据，只包含课程号、课程名、开课院系、学分和状态。
- 课程可以启用或弃用。弃用只影响后续新建课程班，不影响已有课程班、历史选课和历史成绩。
- 课程班是具体开课安排，关联课程、学期、教师、教室、星期、节次、周次和容量。
- 课程班周次使用枚举：`all` 表示全部，`odd` 表示单周，`even` 表示双周。
- 时间冲突检查会同时考虑星期、节次和周次；全部周次与单周/双周均视为冲突，单周和双周可共用同一时段。
